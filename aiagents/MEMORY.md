...
- **v41（2026-08-18）**：e2e 第四次失败（39.01s）。用户再次只贴幻觉分析（称失败在 gpilet 校验、"ray_running flaky"——纯编造：`rayRunning()` 靠 `/proc/1/cmdline` 含 "ray"（bootstrap 脚本文本）即 true，不可能 false；且日志仍未贴出真实错误段）。决定**停止盲猜**：任务脚本增强自带诊断——用 `ray.nodes()` 打印每个节点的 `Alive/NodeManagerAddress/CPU` 并输出 `ALIVE_NODES=`，断言改为 **任务 ≥2 节点 且 alive ≥2**；失败时追加 dump worker 的 `raylet.out` 尾部 + 完整 taskOut。这样若 node-IP 修复（d9f7bf2）未生效，下一次失败输出会直接显示 worker 注册地址是否错误。仍需用户贴出 `ray status` 块之后到 `--- FAIL` 的**原始日志**。
