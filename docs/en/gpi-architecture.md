...
| Cloud abstractions (`sky/clouds`) | `internal/cloud` + Provider interface | One package per Provider; a single struct implements both Provider and `catalog.Source`; `cloud.Register` auto-registers the metadata source; aliyun + aws implemented |
