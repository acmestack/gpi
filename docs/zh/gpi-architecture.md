...
- **SkyPilot 式 bootstrap**：每个节点 Pod 的启动命令自动拉起 **gpilet**（`nohup gpilet serve` 常驻，写 `/var/lib/gpilet/status.json`）与 **Ray**——head 节点 `ray start --head`（监听 6379，dashboard 8265，绑定自身 Pod IP），worker 节点 `ray start --address=<head_pod_ip>:6379`（带重试）。多节点创建为**串行**：head 先建、等待拿到 PodIP，再创建 worker 并注入 head 地址。通过 env 注入 `GPI_ROLE`/`GPI_POD_IP`（downward API）/`GPI_HEAD_ADDR`。
- **默认镜像**：`GetImage` 返回预构建的 `ghcr.io/acmestack/gpi-base:latest`（见 `Dockerfile.gpi-base`，基于 `rayproject/ray` 并内置 gpilet 二进制），可通过 gpi 配置的 `kubernetes.image` 覆盖。
