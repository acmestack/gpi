...
# API 文档（在线交互式查看，无需启动 Swagger UI）
open https://acmestack.github.io/gpi/apis
```

Deployment 默认**不开启** `--docs`（Swagger UI 不随服务暴露）。最新的 OpenAPI 规范已生成并提交在仓库 `docs/apis/openapi.json`（`make openapi` 重新生成），GitHub Pages 交互式预览在 https://acmestack.github.io/gpi/apis；如需在服务上提供 `/swagger.json`，在 `deployment.yaml` 的 `args` 中加回 `--docs`。生产环境可用 Ingress / LoadBalancer 暴露 Service。
